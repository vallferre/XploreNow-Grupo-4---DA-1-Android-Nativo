...
public final class ReservationItem {
    ...
    private static final DateTimeFormatter DATE_SHORT = DateTimeFormatter.ofPattern("dd MMM yyyy", LOCALE);
    ...
    @Nullable public final Double userRating, activityRating, guideRating;
    ...
    private static ReservationItem fromJson(
        ...
        Double guideRating = guideRatingValue >= 0 ? guideRatingValue : null;
        Double userRating = resolveDisplayRating(activityRating, guideRating);
        String reviewComment = firstNonBlank(stringOrNumber(obj, "review_comment"), stringOrNumber(obj, "reviewComment"), stringOrNumber(obj, "comment"));
        String finishedAtValue = firstNonBlank(stringOrNumber(obj, "finished_at"), stringOrNumber(obj, "finishedAt"));
        String ratedAtValue = firstNonBlank(stringOrNumber(obj, "rated_at"), stringOrNumber(obj, "ratedAt"));
    ...
    private static String stringOrNumber(@NonNull JsonObject obj, @NonNull String key) {
        ...
        JsonPrimitive primitive = value.getAsJsonPrimitive();
        return primitive.isString() || primitive.isNumber() ? safe(primitive.getAsString()) : null;
    }

    @NonNull private static String safe(@Nullable String value) { return value != null ? value.trim() : ""; }
    @NonNull private static String firstNonBlank(@Nullable String... values) { if (values == null) return ""; for (String value : values) if (value != null && !value.trim().isEmpty()) return value.trim(); return ""; }
    private static long firstLong(@Nullable Long a,@Nullable Long b,@Nullable Long c,long fallback) { if (a != null) return a; if (b != null) return b; if (c != null) return c; return fallback; }
    private static long firstLong(@Nullable Long a,@Nullable Long b,@Nullable Long c,@Nullable Long d,long fallback) { if (a != null) return a; if (b != null) return b; if (c != null) return c; if (d != null) return d; return fallback; }
    private static double firstDouble(@Nullable Double a,@Nullable Double b,@Nullable Double c,double fallback) { if (a != null) return a; if (b != null) return b; if (c != null) return c; return fallback; }
    private static double firstDouble(@Nullable Double a,@Nullable Double b,double fallback) { if (a != null) return a; if (b != null) return b; return fallback; }
    private static double firstDouble(@Nullable Double a,@Nullable Double b,@Nullable Double c,@Nullable Double d,@Nullable Double e,double fallback) { if (a != null) return a; if (b != null) return b; if (c != null) return c; if (d != null) return d; if (e != null) return e; return fallback; }
    @Nullable private static Double resolveDisplayRating(@Nullable Double activityRating,@Nullable Double guideRating) { if (activityRating != null && guideRating != null) return (activityRating + guideRating) / 2d; if (activityRating != null) return activityRating; return guideRating; }

    @NonNull private String composeSelectedDateTimeLabel() { if (selectedDateLabel.isEmpty()) return ""; return selectedTimeLabel.isEmpty() ? selectedDateLabel : selectedDateLabel + " a las " + selectedTimeLabel; }
    ...
    public boolean canReviewNow() {
        ...
        DateInfo finishedInfo = resolveDateInfo(finishedAtValue);
        if (finishedInfo.epochMillis <= 0L) return false;
        long now = System.currentTimeMillis();
        return now >= finishedInfo.epochMillis && now <= finishedInfo.epochMillis + REVIEW_WINDOW_MILLIS;
    }
    public boolean reviewWindowExpired() {
        if (!ReservationStatus.FINISHED.equals(status) || hasReview()) return false;
        DateInfo finishedInfo = resolveDateInfo(finishedAtValue);
        return finishedInfo.epochMillis > 0L && System.currentTimeMillis() > finishedInfo.epochMillis + REVIEW_WINDOW_MILLIS;
    }
    public boolean matchesQuery(@Nullable String rawQuery) { String normalized = normalizeForSearch(rawQuery); return normalized.isEmpty() || normalizedSearchIndex.contains(normalized); }
    @Nullable public LocalDate localDate() { return localDate; }
    @NonNull public String formattedDateShort() { if (!selectedDateLabel.isEmpty()) return selectedDateLabel; if (scheduledAtValue.isEmpty()) return "Sin fecha"; DateInfo info = resolveDateInfo(scheduledAtValue); return info.localDate == null ? scheduledAtValue : info.localDate.format(DATE_SHORT); }
    @NonNull public String formattedDateLong() {
        String preferred = composeSelectedDateTimeLabel();
        if (!preferred.isEmpty()) return preferred;
        if (scheduledAtValue.isEmpty()) return selectedDateLabel.isEmpty() ? "Sin fecha" : selectedDateLabel;
    ...
    @NonNull public String destinationLabel() { return destination.isEmpty() ? "Destino no informado" : destination; }
    @NonNull public String categoryLabel() { return category.isEmpty() ? "" : ActivityItem.categoryLabel(category); }
    @NonNull public String formattedDuration() {
        if (durationMinutes <= 0) return "Duracion no informada";
        if (durationMinutes % 60 == 0) { long hours = durationMinutes / 60; return hours == 1 ? "1 hora" : hours + " horas"; }
        long hours = durationMinutes / 60; long minutes = durationMinutes % 60;
        if (hours == 0) return minutes + " min";
        return (hours == 1 ? "1 hora" : hours + " horas") + " " + minutes + " min";
    }
    @NonNull public String formattedTotalPrice() {
        double amount = totalPrice > 0 ? totalPrice : pricePerPerson * Math.max(1, participants);
        if (amount <= 0) return "Gratis";
        if ("ARS".equalsIgnoreCase(currency)) return String.format(Locale.getDefault(), "$%.0f", amount);
        return String.format(Locale.getDefault(), "%s %.0f", currency.isEmpty() ? "USD" : currency, amount);
    }
    @NonNull public String cancellationSummary() {
        if (!cancellationDescription.isEmpty()) return cancellationDescription;
        if (cancellationFreeHours > 0) return "Cancelacion sin cargo hasta " + cancellationFreeHours + " h antes.";
        if (!cancellationType.isEmpty()) return "Politica " + cancellationType + ".";
        return "La politica de cancelacion se informara al momento de gestionar la reserva.";
    }
    @NonNull public String cancellationTypeLabel() { return cancellationType.isEmpty() ? "" : ActivityDetail.cancellationTypeLabel(cancellationType); }

    @NonNull
    private static String normalizeForSearch(@Nullable String raw) {
        if (raw == null) return "";
        return Normalizer.normalize(raw.trim(), Normalizer.Form.NFD).replaceAll("\\p{M}+", "").toLowerCase(Locale.ROOT);
    }

    @NonNull
    private static DateInfo resolveDateInfo(@Nullable String raw) {
        String value = safe(raw);
        if (value.isEmpty()) return new DateInfo(0L, null, null);
        try {
    ...
    @NonNull private static String normalizeDateLabel(@Nullable String raw,@NonNull DateInfo fallbackInfo) { String value = safe(raw); if (!value.isEmpty()) { DateInfo info = resolveDateInfo(value); return info.localDate != null ? info.localDate.format(DATE_SHORT) : value; } return fallbackInfo.localDate != null ? fallbackInfo.localDate.format(DATE_SHORT) : ""; }
    @NonNull private static String normalizeTimeLabel(@Nullable String raw,@NonNull DateInfo fallbackInfo) {
        String value = safe(raw);
        if (!value.isEmpty()) {
            DateInfo info = resolveDateInfo(value);
            if (info.zonedDateTime != null) return info.zonedDateTime.format(TIME_ONLY);
            try { return LocalTime.parse(value).format(TIME_ONLY); } catch (DateTimeParseException ignored) {}
            try { return LocalTime.parse(value, DateTimeFormatter.ofPattern("H:mm")).format(TIME_ONLY); } catch (DateTimeParseException ignored) {}
            return value;
        }
